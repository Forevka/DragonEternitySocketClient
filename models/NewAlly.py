...
#
#     result = new_ally_from_dict(json.loads(json_string))
...
class NewAlly:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'NewAlly':
        assert isinstance(obj, dict)
        agr = from_int(obj.get("agr"))
        ...
        pair_state = from_list(lambda x: x, obj.get("pairState"))
        ...
        return NewAlly(agr, bot, bot_tpl_id, dead, effects, evt, gender, hp, id, level, max_hp, max_mp, mp, pair_state, parts, pers_id, round, spell_used_cnt, stand, team)
...
def new_ally_from_dict(s: Any) -> NewAlly:
    return NewAlly.from_dict(s)


def new_ally_to_dict(x: NewAlly) -> Any:
    return to_class(NewAlly, x)
